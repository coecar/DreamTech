...
public class GestorUsuarios {
	...
	public int updateNick(Usuario u, String NickNuevo) throws SQLException{
		String sql = "UPDATE usuarios SET Nick = ? WHERE Nick = ?";
		PreparedStatement pst = agenteBD.getConection().prepareStatement(sql);
		pst.setString(1, NickNuevo);
		pst.setString(2, u.getNick());
		int resultado = agenteBD.update(pst);
		return resultado;
	}
	
	public int updatePass(Usuario u, String ContraseñaNueva) throws SQLException{
		String sql = "UPDATE usuarios SET Password = ? WHERE Nick = ?";
		PreparedStatement pst = agenteBD.getConection().prepareStatement(sql);
	...
	public int updateDatos(Usuario u, String NombreNuevo, String ApellidoNuevo) throws SQLException{
		String sql = "UPDATE usuarios SET Nombre,Apellido = ? WHERE Nick = ?";
		PreparedStatement pst = agenteBD.getConection().prepareStatement(sql);
		pst.setString(1, NombreNuevo);
		pst.setString(1, ApellidoNuevo);
		pst.setString(3, u.getNick());
		int resultado = agenteBD.update(pst);
		return resultado;
	}
